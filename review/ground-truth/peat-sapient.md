...
## 2026-08-31 full sweep delta — `e311e82 → 8861470` (0.1.0; +1 crate)

- **New crate `peat-direct-sapient-cot` [Shipped]** (PR #55; `git diff --stat` 9 files, +1268/-7). A standalone, in-process **mesh-free** CoT↔SAPIENT converter — depends only on `peat-sapient` (`default-features = false`, so no `peat-schema`/CRDT/routing) + `cot-proto 0.5.1` (`peat-direct-sapient-cot/Cargo.toml:9-10`). Bidirectional; the stateful method is CoT→SAPIENT. Public: `DirectConverter::{new, cot_xml_to_sapient_bytes:137}`, free fns `sapient_bytes_to_cot_xml:285` / `sapient_message_to_cot:290` (`src/converter.rs`). **Scoped:** only SAPIENT `DetectionReport` ↔ CoT atom (`a-*`); Registration/Alert/Status/Task + `RangeBearing→CoT` explicitly not implemented (error). Field map (`converter.rs:216-283`): CoT `lat/lon`→SAPIENT `Location.y/x` (axis-swapped), `hae`→`z`, `ce/le`→`x_error/y_error`, `LatLngDegM`/`Wgs84E`, identity as `TrackObjectInfo` kv, report ULID + stable object ULID. Default admission needs a callsign, admits friendly/neutral/unknown. 16 tests, clippy/rustfmt clean; v0.1.0 unpublished. Arch doc `docs/direct-cot-sapient-architecture.md` **Status: Accepted (2026-08-28)** — "additional architecture, not a replacement for the ADR-070 mesh bridge."
- **Workspace now 4 crates** (`Cargo.toml:2`): `peat-sapient`, `peat-mesh-sapient`, `peat-sapient-bridge`, `peat-direct-sapient-cot`.
- **BSI Flex 335 v2.0 unchanged** (package `sapient_msg.bsi_flex_335_v2_0`). **`proto/VERSION` now readable** = submodule pin `244a741 dstl/SAPIENT-Proto-Files` (NOT a semver string) — the prior "unreadable/unverifiable" flag is **resolved**; the "v2.0" designation comes from the package path.
- **Crypto/TLS unchanged.** The new crate has zero crypto (grep `fips|tls|crypto|rustls|aws_lc|kx|X25519|AES` → none). `fips_crypto_provider()` (`peat-sapient/src/connection.rs:337-348`) still restricts cipher suites to AES-GCM but leaves `aws_lc_rs::default_provider()` kx groups (X25519 offered), standard not FIPS module — open todo stands.
- **peat-schema pin:** SAPIENT crates hold caret `0.9.0-rc.30` (`peat-sapient/Cargo.toml:46`, `peat-mesh-sapient/Cargo.toml:25`); umbrella is exact `=0.9.0-rc.34` → ~4-rc lag reopened. ADR-070 still Accepted (`peat/docs/adr/070-...:3`).
