...
## 2026-08-31 full sweep delta — `b4b6ed3 → 6357757` (crate **0.4.22 → 0.5.1**, minor jump)

- **v0.5.0** released the canonical `peat.track.v1.Track` on the sidecar RPCs (BREAKING protobuf; `proto/sidecar.proto:340-343` `PutTrackRequest.canonical_track=2` reserved 1; `:352-355` `GetTracksResponse.canonical_tracks=2`) — the prior "post-0.4.22, crate not bumped / still 0.4.22" framing is now stale. Cross-peer delete propagation released: `delete_document` (`src/node.rs:2459`) mints a `Tombstone` before local remove (`put_tombstone:2474`) and pushes a `TombstoneBatch` (`send_tombstones_to_peer:2490`). Tests `tests/sync_test.rs:347` (propagates + no-resurrect) and `:491` (ignored: re-create-after-delete, still blocked upstream). **RPC 27/27** (`grep 'rpc ' proto/sidecar.proto`).
- **#255 transport-neutral UDP port [Shipped, v0.5.1].** Public `--udp-port` / `PEAT_NODE_UDP_PORT` (`src/main.rs:201`); legacy `--iroh-udp-port` = clap alias; legacy env `PEAT_NODE_IROH_UDP_PORT` promoted at startup (`promote_legacy_udp_port_env`, `:481-486`). **Binary default is ephemeral** — the well-known **51071** is the Ansible/Compose deployment convention (`ansible/roles/peat_node/defaults/main.yml`), not a binary default; Helm `udpPort` defaults null. v0.5.0 runtime predates the neutral setting (needs legacy env).
- **#249 static peering by node ID [Shipped, v0.5.0].** `--peer-node` / `PEAT_NODE_PEER_NODES` take `node_id@host:port` (`src/main.rs:177-184`); `resolve_node_peer_spec` derives the Iroh endpoint via `identity::derive_endpoint_id = HKDF-SHA256(shared_key, "iroh:" + node_id)` (`src/identity.rs:23-24,68-72`). node_id is operator-chosen (default random UUID), **not** SHA-256(Ed25519 pubkey). Distinct from `--peer` (`endpoint_id@host:port`). Offline `derive-id` subcommand.
- **#239–#247 supported Ansible role [Shipped, v0.5.0].** `ansible/roles/peat_node/` — modes `container` (one-directory Compose under `peat_node_container_path`) or `systemd` (signed APT deb822 / DNF repos, service user `peat`, `/var/lib/peat-node` 0750); Vault-backed secrets (`no_log`); attachment `outbox:ro`/`inbox:rw`; static full-mesh from an inventory group (`tasks/topology.yml`). #253 refreshes APT cache before install. Meta: Ubuntu jammy/noble, Debian bookworm, EL 8/9.
- **Versions:** crate 0.5.1 (`Cargo.toml:7`); peat-cli 0.4.5 (`crates/peat-cli/Cargo.toml:3`); mesh pin `=0.9.0-rc.63` (`Cargo.toml:173`, now 3 RCs behind mesh HEAD rc.66); Helm chart still `0.4.10` (`chart/peat-node/Chart.yaml`).
