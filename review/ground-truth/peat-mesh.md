...
## 2026-08-31 full sweep delta — `3d2985e → 0ad275c` (**rc.64 → rc.66**)

- **#396 ADR-076 collection-history ENFORCEMENT [Shipped].** Net-new `src/storage/history_segments.rs` (5355 L) + `src/storage/history_transfer.rs` (1402 L), consuming published `peat_schema::history::v1` (`history_segments.rs:5-11`; pin `peat-schema =0.9.0-rc.34`, `Cargo.toml:40`; `prost 0.13` added). Verified in code+tests: finite SHA-256-sealed segments (`HistorySegmentStore:358`, `SEGMENTS_TABLE`/`EPOCHS_TABLE`, `MaterializedShaMismatch:89`); durability acks (`AuthenticatedDurabilityTarget:135`, `DurabilityProgress`, `AckFrame`/`TAG_ACK=2`); retention enforcement (`evaluate_retention_by_identity:1802`, `EnforcementState::Enforced:3746`); stale-writer fencing (`StaleWriterBehavior::RejectWithActiveEpoch:3179`, `StaleEpoch:1148`); tombstones (`RESERVED_SEGMENT_KEY_PREFIX` + ADR-034 table); admission control (`TransferAdmissionOutcome:154`, bounded `enqueue_bulk_transfer`); metrics (`HistoryMetricsSnapshot:214`, 12 counters); authenticated subordinate transfer (`HistorySegmentProtocol`/`Handler`, formation-auth + optional Ed25519 cert, 401/403 fail-closed).
- **Wire:** NEW dedicated ALPN `CAP_HISTORY_SEGMENT_ALPN = b"peat/history-segment/1"` (`history_transfer.rs:17`; tags SEGMENT=1/ACK=2/NON_DURABILITY=3). **No new `SyncMessageType` byte** — enum still 10 variants 0x00–0x0A (`automerge_sync.rs:105-126`). Canonical router now carries **four** protocols (sync · application-delivery · history-segment · blob-announce).
- **Governing ADRs:** peat-mesh **ADR-0016** `docs/adr/0016-bounded-history-write-and-convergence-policy.md:3` **now Accepted** (was Proposed); new **ADR-0017** (explicit IP bindings, Accepted); ADR-0015 still Proposed; ADR-0014 Accepted. peat-mesh `docs/adr/*.md` = **18** (was 17).
- **#397 explicit multi-interface binding [Shipped].** `IpBindSpec { addr, prefix_len, is_default_route }` (`network/iroh_transport.rs:89`), `bind_with_explicit_ip_bindings:445` (rejects unspecified/multicast, prefix > 32/128, dup, >1 default route/family, `:456-497`), constructor `from_formation_with_discovery_at_ip_bindings:983`. Exact-port mDNS from `endpoint.bound_sockets()` (`:1073-1122`); advertises dialable hex EndpointId as TXT `node_id`; graceful teardown `shutdown_discovery():1163` → `MdnsDiscovery::shutdown` (`discovery/mdns.rs:243`) before endpoint shutdown.
- **Deps:** version `0.9.0-rc.66` (`Cargo.toml:2`); lru `0.16.3→0.18.2` (RUSTSEC-2026-0253, `:148`); temporary schema git patch removed. NOTE a **separate** `[patch.crates-io] peat-btle` git pin (`Cargo.toml:222`) is still present — unrelated, untouched this range.
- **Leader election unchanged — still deterministic, no consensus** (`hierarchy/dynamic_strategy.rs:103` `calculate_leadership_score`, hysteresis 0.1; no raft/paxos/quorum). `src/hierarchy/`, `flat_mesh.rs`, `mesh.rs` untouched this range.
