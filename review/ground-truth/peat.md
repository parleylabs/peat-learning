...
## 2026-08-31 full sweep delta — `7f89476 → cd9e282` (workspace **rc.33 → rc.34**)

- **ADR-076 Reconstructible Collection History Contract** (`docs/adr/076-reconstructible-collection-history-contract.md:3` **Status: Proposed**; `:4` Implementation Approval: approved under peat#1084). Defines seven policy axes (`:38-56`) and a sealed-segment lifecycle Active→Sealed→Durably Acknowledged→Retention Eligible→Removed (`:104-116`), SHA-256 sealed identity (`:119-125`). **peat#1084/#1085/#1086.**
- **`peat_schema::history::v1` = Shipped (rc.34).** `proto/history.proto` (11 messages + 11 enums, package `peat.history.v1`, own header `Version: 0.1.0`); generated types wired `build.rs:25` + exposed `src/lib.rs:190-195`; hand-written validator `src/validation/history.rs` (792 L, conservative resolution — UNSPECIFIED→FullHistory); sync-mode migration `peat-protocol/src/qos/sync_mode.rs:20-64` ("absence never authorizes history loss"); ~19 contract tests `tests/history_contract.rs`; facade `peat-protocol/src/lib.rs:90` → `src/history.rs`. **Enforcement/qualification = Proposed + NEEDS_RUNTIME (peat-sim#77)** — the ADR assigns enforcement to storage/sync impls.
- **#1091 build without system protoc:** `build.rs:29-32` `skip_protoc_run()` + checked-in `proto/peat-schema-descriptor.bin` (229 KB). `cargo build` no longer needs a system `protoc` (only regen does). Air-gapped-CI relevant.
- **#1089 supply-chain:** h2 `0.4.13→0.4.16` (RUSTSEC-2026-0258); lru `→0.18.2` (RUSTSEC-2026-0253); removed temporary peat-mesh git pin (`peat-protocol/Cargo.toml:307` `peat-mesh >=0.9.0-rc.66,<0.9.1`).
- **#1095 Android selected IP bindings** = `docs/adr/076-android-selected-ip-bindings-and-application-relay.md:3` **Status: Accepted**. Vendored `netwatch`.
- **FIPS clean:** no ChaCha20/X25519 in peat source; the only `chacha20` in the diff is the vendored `netwatch/Cargo.lock` (transitive via iroh). AES-256-GCM + ECDH-P256 + Ed25519 + HKDF/HMAC-SHA-256 unchanged. Internal pin `peat-schema =0.9.0-rc.34` (`peat-protocol/Cargo.toml:26`).
- **Stable anchors re-derived (direct `ls`+`grep`):** `peat/docs/adr/*.md` = **83 files** (was 81); **79 numbered ADR files** (was 77) spanning **74 distinct numbers** (5 reused: 023/025/059/064/**076**; highest ADR-076); 4 reference docs unchanged. **ADR-076 is a duplicate number** — one Accepted (android), one Proposed (reconstructible history). **12 Accepted** (was 11; +076-android): {002,009,015,016,023,024,030,041,047,057,070,076-android} — note ADR-015/057 use `**Status:** Accepted` (colon inside the bold, easy to miss with a `**Status**:` regex). `CellRole` still 7 (`peat-protocol/src/models/role.rs`). RPC 27/27. FIPS source posture unchanged.
