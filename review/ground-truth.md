...
| **TAK / CoT TCP** | TLS bridge to a CoT/TAK consumer's wire protocol | **Shipped** | standalone `peat-tak` repo (removed from `peat-transport` at `peat` rc.31, peat#1015 `492fb54`); CoT *translation* stays in `peat-protocol/src/cot/` | ex-`peat-transport/src/tak/` (deleted); `peat-protocol/src/cot/`; consumer pin `peat-sapient/peat-sapient-bridge/Cargo.toml:22` (`peat-tak =0.0.3`); ADR-020/028/029 |
