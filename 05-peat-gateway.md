...
fast-forwarded it to `=0.9.0-rc.40`, so the gateway now lags the ecosystem (`rc.66`) by ~26 RCs.
...
(peat-gateway#151, `Cargo.toml:57,96`). Since then (`4d82282 → a7527c7`) it has moved only on
CI/toolchain — QA-review workflow tweaks (a linked-issue coverage check, peat-gateway#163, and most
recently running Codex QA on the runner, peat-gateway#166), a `rust-analyzer` toolchain component, and
an ignore for the quick-xml `RUSTSEC-2026-0194/0195` advisory — no source change. The `peat-mesh` pin is untouched
